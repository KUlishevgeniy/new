...
def Insert_to_database(file):
    ...
    for index, row in df.iterrows():
        c.execute(
            f"INSERT INTO catalog (art,name,price,brand,category) VALUES ('{row['arts'][1:-2]}','{row['name'][1:-2]}','{str(row['row'][0:-1])}',{row['brand'][1:-2]},{row['cat'][1:-2]})")
        mydb.commit()
...
def ParceStore77Net_EachSection(link):
    ...
    for elem in data:
        ...
        for line in elems:
            if ":" in line:
                property_.append(tuple(
                    [re.sub(r"\s+", " ", line.split(":")[0]), re.sub(r"\s+", " ", line.split(":")[1]).split("//")[0]]))
                # print(property_)
